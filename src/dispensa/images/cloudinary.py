...
if TYPE_CHECKING:
    from dispensa.settings import CloudinaryConfig

# All durable copies live in one Cloudinary folder; the public_id is the
# Instagram shortcode, so re-uploading a shortcode REPLACES its asset. The
# folder name predates the project rename and stays "foodiegram" so existing
# durable URLs keep resolving.
